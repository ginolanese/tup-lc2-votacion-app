...
const $htmlMain = document.getElementById("id-main");
let agregarInforme = ""; // Variable global
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (localStorage.getItem('INFORMES')) {
        ...
        const procesado = guardado_datos.map((element) => element.split("|"));
        console.log(procesado);
        // |${periodosSelect}|${tipoRecuento}|${tipoEleccion}|${cargoSelect}|${distritoSelect}|${$inputSeccionProvincial.value}|${seccionSelect}|${""}|${""}| ${textiCargo}|${textoDistrito}|${textoSeccion}|${tipoEleccion == 1 ? "Pasos" : "Generales"}|${guardarSVG}|${cantidadElectores}|${mesasTotalizadas}|${participacionPorcentaje}
        // Procesar cada informe
        procesado.forEach((partes) => {
            const periodosSelect = partes[1].trim();
            const tipoRecuento = partes[2].trim();
            const tipoEleccion = partes[3].trim();
            const cargoSelect = partes[4].trim();
            const distritoSelect = partes[5].trim();
            const seccionProvincial = partes[6].trim();
            const seccionSelect = partes[7].trim();
            const textiCargo = partes[10].trim();
            const textoDistrito = partes[11].trim();
            const textoSeccion = partes[12].trim();
            const tipoEleccionTexto = partes[13].trim();
            const guardarSVG = partes[14].trim();
            const cantidadElectores = partes[15].trim();
            const mesasTotalizadas = partes[16].trim();
            const participacionPorcentaje = partes[17].trim();
            console.log("Periodo seleccionado:", periodosSelect);
            console.log("Tipo de recuento:", tipoRecuento);
            console.log("Tipo de elección:", tipoEleccion);
            console.log("Cargo seleccionado:", cargoSelect);
            console.log("Distrito seleccionado:", distritoSelect);
            console.log("Sección provincial:", seccionProvincial);
            console.log("Sección seleccionada:", seccionSelect);

            let consultaCompleta;

            if (!seccionProvincial) {
                consultaCompleta = getResultados + `?anioEleccion=${periodosSelect}&tipoRecuento=${tipoRecuento}&tipoEleccion=${tipoEleccion}&categoriaId=${cargoSelect}&distritoId=${distritoSelect}&seccionProvincialId=${seccionProvincial}&seccionId=${seccionSelect}&circuitoId=&mesaId=`
            ...
            fetch(consultaCompleta)
                ...
                .then((datos) => {
                    console.log(datos);
                    //2023 > Pasos > PRESIDENTE/A > Ciudad Autónoma de Buenos Aires > Comuna 02
                    let tablasAgruppacion = ""
                    // Agregar datos al informe dinámicamente
                    datos.valoresTotalizadosPositivos.forEach((valor) => {
                        tablasAgruppacion += `
                        <div class="tabla-agrupacion">
                            <p>${valor.nombreAgrupacion}</p>
                            <p class="tabla-por">${valor.
                                votosPorcentaje}%<br /><br />${valor.votos} votos</p>
                        </div>
                    ...
                    agregarInforme += ` <tbody id="tarjeta">
                    ...
                    <tr>
                        ...
                        <td>
                            <p class="texto-elecciones-chico">Elecciones ${periodosSelect} | ${tipoEleccionTexto}</p>
                            <p class="texto-path-chico">${periodosSelect} >${tipoEleccionTexto} >${textiCargo} >${textoDistrito}>${textoSeccion}</p>
                        </td>
                        <td>
                            <div class="paso-gen">
                                <div class="mesa-comp">
                ...
                M382.635,358.347c23.575,0,43.614,24.754,47.15,57.76h-94.302C339.02,383.101,359.059,358.347,382.635,358.347z">
                                                ...
                                                </svg>Mesas<br />Estructuadas<br />${cantidadElectores}</span></p>
                                    </a>
                                </div>
                                <div class="electores">
                            ...
                            c-0.238-0.571-0.797-0.944-1.416-0.944l0,0c-4.601,1.534-7.55-4.364-8.966-9.319c-2.595-10.027-1.18-14.864,0.473-17.105H180.521z">
                                                ...
                                                </svg>Electorales<br />${mesasTotalizadas}</span></p>
                                    </a>
                ...
                <tbody>`
                    ...
                    // Actualizar el contenido HTML dinámicamente
                    $htmlMain.innerHTML = `
                ...
                .catch((error) => {
                    console.error(error);
                    mostrarMensaje($mensajeRojo, "Error: El servicio se ha caído. Intente más tarde");
                });
        });
    } else {
        mostrarMensaje($mesajeAmarillo, "No hay informes guardados para mostrar", 2000);
    }
});

// Función para mostrar mensajes
function mostrarMensaje(msj, cadena, tiempo = 4000) {
    const mensajeParrafo = msj.querySelector("p");
    if (mensajeParrafo) {
        mensajeParrafo.textContent = cadena;
